...
def get_data_type(url: str, text: str) -> str:
    has_url  = bool(url  and url.strip()  and url  != "nan" and len(url)  > 3)
    has_text = bool(text and text.strip() and text != "nan" and len(text) > 10)
    if has_text and not has_url:  return "text"
    if has_url  and not has_text: return "url"
    if has_url  and has_text:     return "both"
    return "unknown"

def extract_features(url: str, text: str, feature_cols: list):
    url  = str(url  or "")
    text = str(text or "")
    f    = {}
